...
describe('Test for DELETE method on Tutors', () => {
  ...
  it('Should delete a tutor with correct authentication and id', async () => {
    const response = await request(app).delete(`/tutor/${tutorId}`).set('Authorization', authToken);
  ...
  it('Should return 400 if tutor have a pet associeted', async () => {
    ...
    const DeleteResponse = await request(app).delete(`/tutor/${tutorId}`).set('Authorization', authToken);
  ...
  it('Should return 401 if it does have invalid authentication', async () => {
    const response = await request(app).delete(`/tutor/${tutorId}`).set('Authorization', 'Bearer 123');
